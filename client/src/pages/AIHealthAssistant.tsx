...
const AIHealthAssistant = () => {
  ...
  const [apiStatus, setApiStatus] = useState<'loading' | 'ready' | 'error' | 'not_configured'>('loading');
  ...
  useEffect(() => {
    const checkApiStatus = async () => {
      try {
        const response = await axios.get('/api/ai/health-assistant-public');
        setApiStatus(response.data.status === 'ready' ? 'ready' : 'not_configured');
      } catch (error) {
  ...
  const getAIResponse = async (userInput: string) => {
    try {
      ...
      const healthCheck = await axios.get('/api/ai/health-assistant-public')
        .catch(err => {
          console.warn('AI Health check failed:', err);
          return { data: { status: 'unavailable' } };
        });
      
      if (healthCheck.data.status === 'not_configured') {
        return `I apologize, but the AI health assistant is currently unavailable due to configuration issues with the API key. Please contact the administrator for assistance.`;
      }
      ...
      return 'I\'m sorry, I encountered an error while processing your request. The server may be experiencing issues or is not properly configured. Please try again later.';
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">
                AI Service Status: {apiStatus === 'loading' ? 'Checking...' : apiStatus === 'not_configured' ? 'Not Fully Configured' : 'Connection Error'}
              </h3>
              <div className="mt-2 text-sm text-yellow-700">
                <p>
                  {apiStatus === 'loading' 
                    ? 'Checking connection to AI service...' 
                    : apiStatus === 'not_configured'
                    ? 'The AI health assistant is not properly configured. Some features may be limited. Please contact your administrator.' 
                    : 'Unable to connect to the AI service. The service may be down or experiencing issues.'}
                </p>
              </div>
            </div>
          </div>
