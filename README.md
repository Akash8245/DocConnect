...
For the frontend, create `client/.env` (or `.env.local`) and provide the backend URL that should be used outside of local proxying:

```
VITE_API_BASE_URL=http://localhost:5001
```

When `VITE_API_BASE_URL` is defined the React app will talk to that URL directly; otherwise it continues to use the Vite dev proxy.
